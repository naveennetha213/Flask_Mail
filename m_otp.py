...
def verification_(x, y):
    ...
    response = requests.post("https://api.tiniyo.com/v1/Account/" + auth_id + "/Verifications", data=data,
                             headers=headers)
    return response
...
def verification_check(x, y, otp):
    ...
    data = json.dumps(c)
    headers = header(auth_id, secret_auth_id)
    response = requests.post("https://api.tiniyo.com/v1/Account/" + auth_id + "/VerificationsCheck", data=data,
                             headers=headers)
    data = response.json()
